...
class SchemaMigrator:
  ...
  def extract_create_table_info(self, create_statement: str) -> Tuple[str, List[Dict]]:
    ...
    table_match = re.search(
        r'CREATE TABLE `([^`]+)`', create_statement, re.IGNORECASE)
    if not table_match:
      return None, []
    ...
    field_section_match = re.search(
        r'CREATE TABLE.*?\((.*?)\)(?:\s+ENGINE)', create_statement, re.DOTALL | re.IGNORECASE)
    ...
    for field in fields:
      ...
      if has_korean and english_field:
        ...
        new_line = original_line.replace(
            f'`{field_name}`', f'`{new_field_name}`')
        if comment:
          new_line = re.sub(
              r"COMMENT\s+['\"][^'\"]*['\"]", f"COMMENT '{new_comment}'", new_line)

        migrated_statement = migrated_statement.replace(
