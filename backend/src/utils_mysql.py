...
def get_mysql_config():
    ...
    """
    return dict(
        host=os.environ.get('MYSQL_HOST', '1.2.3.4'),
...
def _valStr(val):
    ...
    return "'" + val.replace("'", "\\'").replace('"', '\\"') + "'" if type(val) is str else val
...
class Mysql(object):
    ...
    def fetch(self, sql, limit=0, close=False):
        ...
        if close:
            self.conn.close()
        return curs.fetchall() if limit == 0 else curs.fetchone() if limit == 1 else curs.fetchmany(limit)
    ...
    def insert_one_by_dict(self, table_name, dic, close=False):
        ...
        Args:
            ...
            close (bool): 쿼리 실행 후 연결 종료 여부
        """
        sql = f"INSERT IGNORE INTO {table_name} ({_join_keys(dic.keys)}) VALUES ({_join_vals(dic.values)});"
        self.exec(sql, close)
    ...
    def insert_one(self, table_name, csv, close=False):
        ...
        Args:
            ...
            close (bool): 쿼리 실행 후 연결 종료 여부
        """
        sql = f"INSERT IGNORE INTO {table_name} ({_join_keys(csv[0])}) VALUES ({_join_vals(csv[1])});"
        self.exec(sql, close)
    ...
    def insert(self, table_name, data, close=False, inType="csv"):
        ...
        sql = f"INSERT IGNORE INTO {table_name} ({_join_keys(csv[0])}) VALUES {values};"
        self.exec(sql, close)
    ...
    def distinct_field(self, table_name, field):
        ...
        Returns:
            list: 고유값 목록
        """
        return [d[field] for d in self.query(f"select distinct({field}) from {table_name}")]
    ...
    def update(self, table_name, update_data, where_condition=None, close=False):
        ...
        Args:
            ...
            close (bool): 쿼리 실행 후 연결 종료 여부
        """
        set_clause = ", ".join([f"{_keyStr(k)} = {_valStr(v)}" for k, v in update_data.items()])
