...
def sample_sql_query():
    return {
        "sql": "SELECT nid, title FROM notice_list WHERE posted_date >= '2024-01-01' ORDER BY nid DESC LIMIT 10"
    }
...
def sample_search_data():
    return {
        ...
        "base_sql": "SELECT notices.nid, notices.posted_date, notices.org_name, notices.title, notices.detail_url FROM notices",
...
def sample_search_result():
    return [
        (1, "2024-01-01", "테스트기관", "안전점검 공고", "http://test.com/1"),
        (2, "2024-01-02", "테스트기관2", "안전진단 공고", "http://test.com/2")
    ]

class TestFetchBySqlEndpoint:
    
    def test_fetch_by_sql_success(self, mock_mysql, sample_sql_query, sample_notice_data):
        """SQL 쿼리 실행 성공 테스트"""
...
class TestNoticeListBySearchEndpoint:
    ...
    def test_notice_list_by_search_success(self, mock_mysql, sample_search_data, sample_notice_data, sample_search_result):
        """키워드 검색 성공 테스트"""
    ...
    def test_notice_list_by_search_no_keywords_found(self, mock_mysql, sample_search_data):
    ...
    def test_notice_list_by_search_below_min_point(self, mock_mysql, sample_search_data):
        """최소 점수 미달로 필터링되는 경우 테스트"""
        # 낮은 가중치로 검색 결과 생성 (최소 점수 미달)
        search_data_low_score = sample_search_data.copy()
        search_data_low_score["keywords"] = "테스트*1,키워드*1"  # 총 점수 2점 (min_point 4점 미달)
        ...
        response = client.post("/notice_list_by_search/", json=search_data_low_score)
        
        assert response.status_code == 200
        assert response.json() == []

    def test_notice_list_by_search_with_not_filter(self, mock_mysql, sample_search_data):
        """제외 키워드가 적용되는 경우 테스트"""
    ...
    def test_notice_list_by_search_with_add_where_condition(self, mock_mysql, sample_search_data):
