...
def mock_mysql_functions():
    ...
    with ExitStack() as stack:
        mock_settings_list = stack.enter_context(patch('server_bid.find_settings_notice_list'))
        mock_settings_by_name = stack.enter_context(patch('server_bid.find_settings_notice_list_by_name'))
        mock_settings_detail = stack.enter_context(patch('server_bid.find_settings_notice_detail'))
        mock_settings_detail_by_name = stack.enter_context(patch('server_bid.find_settings_notice_detail_by_name'))
        mock_detail_config = stack.enter_context(patch('server_bid.detail_config_by_name'))
        mock_notice_list = stack.enter_context(patch('server_bid.find_notice_list_with_category'))
        mock_notice_by_category = stack.enter_context(patch('server_bid.find_notice_list_by_category'))
        mock_notice_statistics = stack.enter_context(patch('server_bid.find_notice_list_for_statistics'))
        mock_last_notice = stack.enter_context(patch('server_bid.find_last_notice'))
        mock_search_notice = stack.enter_context(patch('server_bid.search_notice_list'))
        mock_upsert_notice = stack.enter_context(patch('server_bid.upsert_notice_list'))
        mock_update_status = stack.enter_context(patch('server_bid.update_notice_status'))
        mock_find_bids = stack.enter_context(patch('server_bid.find_my_bids'))
        mock_find_bids_status = stack.enter_context(patch('server_bid.find_my_bids_by_status'))
        mock_find_logs = stack.enter_context(patch('server_bid.find_logs_notice_scraping'))
        mock_find_errors = stack.enter_context(patch('server_bid.find_errors_notice_scraping'))
        mock_scrape_list = stack.enter_context(patch('server_bid.scrape_list'))
        mock_delete_old = stack.enter_context(patch('server_bid.delete_old_notice_list'))
        mock_backup = stack.enter_context(patch('server_bid.backup_db'))
        mock_all_categories = stack.enter_context(patch('server_bid.find_all_settings_notice_category'))
        mock_category = stack.enter_context(patch('server_bid.find_settings_notice_category'))
        mock_search_weight = stack.enter_context(patch('server_bid.get_search_weight'))
        mock_filter_not = stack.enter_context(patch('server_bid.filter_by_not'))
        mock_keyword_weights = stack.enter_context(patch('server_bid.get_keyword_weight_list'))
        mock_add_settings = stack.enter_context(patch('server_bid.add_settings_to_notice'))
...
class TestSettingsNoticeListEndpoints:
    ...
    def test_get_settings_notice_list_by_name_success(self, mock_mysql_functions, sample_settings_data):
        mock_mysql_functions['settings_by_name'].return_value = sample_settings_data
        
        response = client.get("/settings_notice_list/테스트기관")
        assert response.status_code == 200
        assert response.json()["org_name"] == "테스트기관"

    def test_get_settings_notice_list_by_name_not_found(self, mock_mysql_functions):
        mock_mysql_functions['settings_by_name'].return_value = None
        
        response = client.get("/settings_notice_list/존재하지않는기관")
        assert response.status_code == 404

    def test_upsert_settings_notice_list_success(self, mock_mysql_functions, sample_settings_data):
        mock_mysql_functions['settings_by_name'].return_value = sample_settings_data
        
        response = client.post("/settings_notice_list/테스트기관", json=sample_settings_data)
        assert response.status_code == 200
...
class TestSettingsNoticeDetailEndpoints:
    ...
    def test_get_all_settings_notice_detail_success(self, mock_mysql_functions):
        mock_mysql_functions['settings_detail'].return_value = [
            {"org_name": "기관1", "title": "xpath1"},
            {"org_name": "기관2", "title": "xpath2"}
        ]
...
class TestNoticeListEndpoints:
    ...
    def test_get_notice_list_gap_default(self, mock_mysql_functions, sample_notice_data):
    ...
    @patch('server_bid.DAY_GAP', '7')
    def test_get_notice_list_gap_custom(self, mock_mysql_functions, sample_notice_data):
        mock_mysql_functions['notice_list'].return_value = sample_notice_data
        mock_mysql_functions['add_settings'].return_value = None
    ...
    @patch('server_bid.DAY_GAP', '7')
    def test_get_notice_list_gap_category(self, mock_mysql_functions, sample_notice_data):
        mock_mysql_functions['notice_by_category'].return_value = sample_notice_data
        mock_mysql_functions['add_settings'].return_value = None
    ...
    def test_get_notice_list_for_statistics(self, mock_mysql_functions):
        mock_mysql_functions['notice_statistics'].return_value = [
            {"org_name": "기관1", "posted_date": "2024-01-01", "category": "공사점검"}
        ]
    ...
    def test_get_last_notice_success(self, mock_mysql_functions):
        mock_mysql_functions['last_notice'].return_value = {
            "title": "최근 공고"
        }
    ...
    def test_search_notice_list_success(self, mock_mysql_functions, sample_notice_data):
...
class TestLogsAndErrorsEndpoints:
    ...
    def test_get_logs_success(self, mock_mysql_functions):
        mock_mysql_functions['find_logs'].return_value = [
            {"id": 1, "message": "스크래핑 성공", "created_at": "2024-01-01 10:00:00"}
        ]
    ...
    def test_get_errors_success(self, mock_mysql_functions):
        mock_mysql_functions['find_errors'].return_value = [
            {"id": 1, "error_message": "스크래핑 실패", "created_at": "2024-01-01 10:00:00"}
        ]
...
class TestKeywordEndpoints:
    ...
    def test_get_all_categorys_success(self, mock_mysql_functions):
        mock_mysql_functions['all_categories'].return_value = [
            {"category": "공사점검", "keywords": "안전,점검"},
            {"category": "성능평가", "keywords": "성능,평가"}
        ]
    ...
    def test_get_categorys_by_category_success(self, mock_mysql_functions):
        mock_mysql_functions['category'].return_value = [
            {"sn": 1, "keywords": "안전,점검", "category": "공사점검"}
        ]
    ...
    def test_search_by_keyword_weight_success(self, mock_mysql_functions, sample_notice_data):
    ...
    def test_filter_notice_list_success(self, mock_mysql_functions, sample_notice_data):
    ...
    def test_parse_keyword_weights_success(self, mock_mysql_functions):
        mock_mysql_functions['keyword_weights'].return_value = [
            ("안전", 3),
            ("점검", 2),
            ("진단", 1)
        ]
        
        response = client.get("/parse_keyword_weights?keyword_weight_str=안전*3,점검*2,진단*1")
        assert response.status_code == 200
...
class TestErrorHandling:
    ...
    def test_settings_exception_handling(self, mock_mysql_functions):
        mock_mysql_functions['settings_list'].side_effect = Exception("Database error")
        
        response = client.get("/settings_notice_list")
        assert response.status_code == 500

    def test_notice_list_exception_handling(self, mock_mysql_functions):
        mock_mysql_functions['notice_list'].side_effect = Exception("Database error")
        
        response = client.get("/notice_list")
        assert response.status_code == 500

    def test_search_exception_handling(self, mock_mysql_functions):
        mock_mysql_functions['search_notice'].side_effect = Exception("Search error")
