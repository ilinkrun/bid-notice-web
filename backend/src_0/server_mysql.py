...
def post_notices_by_search(search: Search):
    ...
    searched_by_keyword_arr = [{nid: row} for (nid, row) in rst.items() if row["point"] >= int(min_point)]
    ...
    nids_after_keyword_arr = [list(row.keys())[0] for row in searched_by_keyword_arr]
    nids_after_not_arr = [nid for nid in nids_after_keyword_arr if nid not in nids_in_not_arr]  # ? 검색 최종 결과 nids
