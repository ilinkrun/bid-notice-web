...
# SETTINGS_NOTICE_LIST_FIELDS = ["기관명", "url", "iframe", "rowXpath", "paging", "startPage", "endPage", "login", "제목", "상세페이지주소", "작성일"]
...
def get_search_weight(keyword_weight_str, min_point=4, add_where="`작성일` > '2023-03-31'"):
    ...
    for (keyword, weight) in keyword_weights:
        ...
        for r in rs:
            nid = r[2]
            if not nid in rsts:
                rsts[nid] = {KEY_FIELD_FOR_SEARCH: r[3], "matched": r[0], "point": r[1]}
            else:
...
# * 검색어 설정 -> nid 배열
def find_nids_by_search_setting(keyword_weight_str, nots_str, min_point, add_where=""):
    ...
    dicts = get_search_weight(keyword_weight_str, min_point=min_point, add_where=add_where)
...
def get_search_results(keyword_weight_str, nots_str, min_point, add_where=""):
    ...
    for rst in rsts:
        for (nid, row) in rst.items():
            response.append([row["matched"], row["point"], nid, row[KEY_FIELD_FOR_SEARCH]])
...
def find_nids_in_table(nids, table_name, exist=True):
    ...
    dones = [] if rs == None or len(rs) == 0 else [r[0] for r in mysql.fetch(sql)]
    # dones = [r[0] for r in mysql.fetch(sql)]

    if exist: # 존재하는 경우
        return dones
    else: # table_name에는 존재하지 않는 경우
        return [nid for nid in nids if not nid in dones]
...
# * 
def find_notice_list_by_search(keyword_weight_str, nots_str, min_point, add_where="", base_sql="", add_sql=""):
    print(f"@@@ find_notice_list_by_search:: keyword_weight_str: {keyword_weight_str}, nots_str: {nots_str}, min_point: {min_point}, add_where: {add_where}, base_sql: {base_sql}, add_sql: {add_sql}")
    nids = find_nids_by_search_setting(keyword_weight_str, nots_str, min_point, add_where)
    if nids == []:
        return []
