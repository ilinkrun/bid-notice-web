...
// 아바타 파일 저장
const saveAvatarFile = (userId, svgContent) => {
    try {
        // 디렉토리가 없으면 생성
...
// 사용자 아바타 생성 및 저장
const generateAndSaveAvatar = (user) => {
    if (user.avatar) {
...
export const authResolvers = {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!user.avatar) {
                        ...
                        // 업데이트된 사용자 정보 다시 읽기
                        const updatedUsers = readJsonFile(USERS_FILE);
    ...
    Mutation: {
        ...
        updateUserProfile: async (_, { token, input }) => {
            try {
                const sessions = readJsonFile(SESSIONS_FILE);
                const session = sessions.find(s => s.token === token && new Date(s.expiresAt) > new Date());
                if (!session) {
        ...
        updateUserPassword: async (_, { token, currentPassword, newPassword }) => {
            try {
                const sessions = readJsonFile(SESSIONS_FILE);
                const session = sessions.find(s => s.token === token && new Date(s.expiresAt) > new Date());
                if (!session) {
