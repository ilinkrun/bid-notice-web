...
const markdownSourceStore = new Map();
const getMarkdownStoreKey = (board, postId) => {
    return `${board}:${postId}`;
};
export const boardsResolvers = {
    Query: {
        boardsPostsAll: async (_, { board }) => {
            try {
        ...
        boardsPostsOne: async (_, { id, board }) => {
            try {
                console.log(`게시글 상세 조회 요청: ${board}, ${id}`);
                // API 호출
                const response = await boardApiClient.get(`/posts/${board}/${id}`);
        ...
        boardsCommentsAll: async (_, { board, post_id, page = 1, per_page = 50 }) => {
            try {
                console.log(`댓글 목록 조회 요청: ${board}, ${post_id}, page=${page}, per_page=${per_page}`);
                // API 호출
                const response = await boardApiClient.get(`/comments/${board}/${post_id}?page=${page}&per_page=${per_page}`);
        ...
        boardsCommentsOne: async (_, { id }) => {
            try {
                console.log(`댓글 상세 조회 요청: ${id}`);
                // API 호출
                const response = await boardApiClient.get(`/comments/${id}`);
                return response.data;
            }
            catch (error) {
    ...
    Mutation: {
        boardsPostCreate: async (_, { board, input }) => {
            try {
        ...
        boardsPostUpdate: async (_, { board, input }) => {
            try {
                console.log(`게시글 수정 요청: ${board}, ${input.id}`, input);
                // API 호출
                const response = await boardApiClient.put(`/posts/${board}/${input.id}`, {
                    title: input.title,
                    content: input.content,
        ...
        boardsPostDelete: async (_, { board, input }) => {
            try {
        ...
        boardsCommentCreate: async (_, { input }) => {
            try {
                console.log('댓글 생성 요청:', input);
                // API 호출
                const response = await boardApiClient.post('/comments', {
                    board: input.board,
        ...
        boardsCommentUpdate: async (_, { input }) => {
            try {
                console.log(`댓글 수정 요청: ${input.id}`, input);
                // 먼저 기존 댓글 정보 조회
                const getResponse = await boardApiClient.get(`/comments/${input.id}`);
                ...
                // API 호출
                const response = await boardApiClient.put(`/comments/${input.id}`, {
                    content: input.content,
                    password: input.password,
        ...
        boardsCommentDelete: async (_, { input }) => {
            try {
                ...
                const existingComment = getResponse.data;
                // API 호출
                const response = await boardApiClient.delete(`/comments/${input.id}`, {
