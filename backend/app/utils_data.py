...
def find_folder_by_category(category="공사점검", full=True):
   ...
   folder = "24_공사점검" if not f"foler_{category}" in config else config[f"foler_{category}"]
...
def find_folders(root_path, search = '', recursive = False):
   return [dir.replace(f'{root_path}/', '') for dir in _find_directories(root_path, search, recursive)]
