...
async function getNaraNoticesByCategory(category: string, gap: number): Promise<Notice[]> {
  try {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  try {
    ...
    return {
      ...
      description: `나라장터 업무와 무관한 입찰공고 목록입니다.`,
    };
  } catch (error) {
    console.error('Error generating metadata:', error);
    return {
      title: '나라장터(무관) | ILMAC BID',
      description: '나라장터 업무와 무관한 입찰공고 목록입니다.',
    };
  }
}
...
export default async function NaraIrrelevantPage({ params, searchParams }: PageProps) {
  ...
  try {
    ...
    const notices = await getNaraNoticesByCategory('무관', gap);
    ...
    console.error('Error in NaraIrrelevantPage:', error);
    return (
