...
export default async function DonePage({ params, searchParams }: PageProps) {
  ...
  if (!resolvedSearchParams.gap) {
    redirect(`/notices/gov/done?category=${defaultCategories}&gap=7`);
  }

  // category 파라미터가 없으면 기본값으로 리디렉션
  if (!resolvedSearchParams.category) {
    redirect(`/notices/gov/done?category=${defaultCategories}&gap=${resolvedSearchParams.gap}`);
  }
  ...
  try {
    ...
    console.error('Error in DonePage:', error);
    return (
